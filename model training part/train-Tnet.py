...
def get_args():
    ...
    parser.add_argument('--saveDir', default='./training model result', help='model save dir')
    parser.add_argument('--trainData', default='human_matting_data', help='train dataset name')
    parser.add_argument('--trainList', default='./data/train.txt', help='training img name list')
    parser.add_argument('--load', default= 'portrait_matting', help='save model')
    parser .add_argument('--gpus', default='0,1,2,3', help='gpus number') #added

    parser.add_argument('--finetuning', action='store_true', default=False, help='finetuning the training') # lyk If this is your first training, then it would be False rather than True.
    ...
    parser.add_argument('--train_batch', type=int, default=11, help='input batch size for train')
...
class Train_Log():
    ...
    def load_model(self, model):
        ...
        lastest_out_path = "{}/ckpt_lastest.pth".format(self.save_dir_model)
        ckpt = torch.load(lastest_out_path)  #checkpoint
